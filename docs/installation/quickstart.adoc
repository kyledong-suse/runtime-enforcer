...
Runtime-Enforcer ships with a built-in OTEL collector sidecar that handles violation metrics and trace forwarding.
...
runtime-enforcer   runtime-enforcer-agent-lsxt4                         2/2     Running
```

[NOTE]
====
Each agent pod now runs two containers: the `agent` and the `otel-collector` sidecar.
====

=== Verify violation metrics

The OTEL collector sidecar exposes Prometheus metrics on port 9090. To verify:

```bash
kubectl port-forward -n runtime-enforcer ds/runtime-enforcer-agent 9090:9090
...
In one terminal, check the OTEL collector sidecar logs:

```bash
kubectl logs -n runtime-enforcer ds/runtime-enforcer-agent -c otel-collector -f
```
...
In the otel-collector sidecar logs you should see a trace for the `ps` command being executed.
...
The sidecar collector will forward traces to the configured endpoint while continuing to process violation logs into Prometheus metrics locally.

=== Disable the sidecar and use an external collector directly

If you already have an OTEL collector deployed in your cluster, you can disable the sidecar and point the agent directly at it:
